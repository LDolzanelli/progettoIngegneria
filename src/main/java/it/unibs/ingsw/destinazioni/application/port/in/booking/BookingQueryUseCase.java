...
public interface BookingQueryUseCase {
  ...
  /*@ requires visit != null && visit.getId() != null;
    ...
    @ signals (VisitDayException e) visit.getId() == null;
    @ pure
    @*/
  ...
  /*@ requires user != null && user.getId() != null;
    ...
    @ signals (UserException e) user.getId() == null;
    @ pure
    @*/
  ...
  /*@ requires bookingCode != null && !bookingCode.trim().isEmpty();
    ...
    @ signals (BookingException e) bookingCode == null ||
  ...
  /**
   * Ottiene la visita associata a una prenotazione tramite il codice di
   * prenotazione.
   * 
   * @param bookingCode codice della prenotazione
   * @return la visita associata alla prenotazione
   * @throws VisitDayException se la visita non viene trovata
   */
  /*@ requires bookingCode != null && !bookingCode.trim().isEmpty();
    @ ensures \result != null;
    @ ensures getBookingsByVisit(\result).stream().anyMatch(b ->
    @         b.getBookingCode().equals(bookingCode));
    @ signals (VisitDayException e) bookingCode == null ||
    @                                     bookingCode.trim().isEmpty();
    @ pure
    @*/
