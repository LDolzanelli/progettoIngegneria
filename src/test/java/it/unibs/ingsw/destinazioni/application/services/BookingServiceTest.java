...
class BookingServiceTest {
    ...
    @BeforeEach
    void setUp() {
        bookingRepositoryMock = Mockito.mock(BookingRepositoryPort.class);
        visitRepositoryMock = Mockito.mock(VisitRepositoryPort.class);
        getUserInfoUseCase = Mockito.mock(GetUserInfoUseCase.class);
        ...
        Mockito.when(visitRepositoryMock.findAll()).thenReturn(Set.of(testVisit));
    }
    ...
    void bookVisit_ShouldThrowExceptionWhenWrongUserTriesBooking() {
        ...
        Mockito.when(getUserInfoUseCase.findById(user1.getId())).thenReturn(Optional.of(user1));
        Mockito.when(getUserInfoUseCase.findById(user2.getId())).thenReturn(Optional.of(user2));

        Mockito.when(user1.getRole()).thenReturn(Role.VOLUNTEER);
        Mockito.when(user2.getRole()).thenReturn(Role.CONFIGURATOR);
